...
```
trim()	删除字符串两端的空格或其他预定义字符	 echo trim("\r\n123\r\n");
rtrim()	删除字符串右边的空格或其他预定义字符	 echo rtrim("123\r\n");"
chop()	rtrim()的别名	同上                  echo chop("123\r\n")
ltrim()	删除字符串左边的空格或其他预定义字符	 echo ltrim("\r\n123");
```

```
dirname()	回路径中的目录部分   echo dirname("c:/testweb/home.php");    c:/testweb
```

```
str_pad()	把字符串填充为指定的长度	echo str_pad('123',5,".");    123..
str_repeat()	重复使用指定字符串	echo str_repeat(".",5);    .....
str_split()	把字符串分割到数组中	print_r(str_split("123"));  [1,2,3]
strrev()	反转字符串	echo strrev("123");    321
```
...
var_dump(explode('.','111@qq.com')) ;   ['111@qq','com']
